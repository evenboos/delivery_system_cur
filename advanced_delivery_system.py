...
class PartitionAlgorithm:
    ...
    def capacity_based_partition(customers, vehicle_capacity=50):
        ...
        sorted_customers = sorted(
            customers, key=lambda c: (c.priority != "urgent", c.cargo_weight)
        )
...
class RouteOptimizer:
    ...
    def genetic_algorithm_tsp(
        customers, population_size=50, generations=100, mutation_rate=0.1
    ):
        """遗传算法TSP"""
        if not customers:
            return [], 0
        ...
        for generation in range(generations):
            # 计算适应度（距离越短适应度越高）
            fitness_scores = [
                (1.0 / (calculate_distance(ind) + 1), ind) for ind in population
            ]
    ...
    def ant_colony_optimization(
        customers, num_ants=20, iterations=50, alpha=1.0, beta=2.0, evaporation=0.5
    ):
        """蚁群算法TSP"""
        if not customers:
            return [], 0
        ...
        for iteration in range(iterations):
            ...
            for route, distance in all_routes:
                ...
                for i in range(len(route) - 1):
                    ...
                    pheromones[route[i + 1]][route[i]] += deposit        # 转换为坐标路径
        result_route = [points[i] for i in best_route]
        return result_route, best_distance

    @staticmethod
    def simulated_annealing_tsp(
        customers, initial_temp=1000, cooling_rate=0.95, min_temp=1
    ):
        """模拟退火算法TSP"""
        if not customers:
            return [], 0
        ...
        def get_neighbor(route):
            ...
            # 处理边界情况：如果客户数量太少，直接返回原路径
            available_range = range(1, len(route) - 1)
            if len(available_range) < 2:
                return new_route
            
            i, j = random.sample(available_range, 2)  # 不包括起始和结束的原点
...
class DeliveryZone:
    ...
    def optimize_route(self, algorithm="greedy"):
        ...
        if algorithm == "greedy":
            self.route, self.total_distance = RouteOptimizer.greedy_nearest(
                self.customers
            )
            self.algorithm_used = "贪心最近邻"
        elif algorithm == "2opt":
            greedy_route, _ = RouteOptimizer.greedy_nearest(self.customers)
            self.route, self.total_distance = RouteOptimizer.two_opt_improve(
                greedy_route, self.customers
            )
            self.algorithm_used = "2-opt优化"
        elif algorithm == "dp":
            self.route, self.total_distance = RouteOptimizer.dynamic_programming_tsp(
                self.customers
            )
            self.algorithm_used = "动态规划(DP)"
        elif algorithm == "ga":
            self.route, self.total_distance = RouteOptimizer.genetic_algorithm_tsp(
                self.customers
            )
            self.algorithm_used = "遗传算法(GA)"
        elif algorithm == "aco":
            self.route, self.total_distance = RouteOptimizer.ant_colony_optimization(
                self.customers
            )
            self.algorithm_used = "蚁群算法(ACO)"
        elif algorithm == "sa":
            self.route, self.total_distance = RouteOptimizer.simulated_annealing_tsp(
                self.customers
            )
...
class AdvancedCalculationThread(QThread):
    ...
    def run(self):
        ...
        if self.partition_method == "分治法":
            ...
            customer_groups = PartitionAlgorithm.kmeans_partition(
                customers, num_clusters
            )
        ...
        for i, group in enumerate(customer_groups):
            if group:  # 确保组不为空
                ...
                distance = zone.optimize_route(
                    self.route_method.lower().replace("-", "")
                )
                total_distance += distance
                zones.append(zone)
...
class AdvancedDeliverySystemWindow(QMainWindow):
    ...
    def on_advanced_calculation_finished(self, zones, total_distance, stats):
        ...
        result_text += (
            f"距离范围: {stats['min_distance']:.2f} - {stats['max_distance']:.2f}\n"
        )
    ...
    def run_single_comparison(self):
        ...
        partition_method, route_method = self.comparison_algorithms[
    ...
    def on_comparison_finished(self, zones, total_distance, stats):
        ...
        self.comparison_results.append(
            {"zones": zones, "total_distance": total_distance, "stats": stats}
        )
        ...
        self.comparison_table.setItem(
            row, 0, QTableWidgetItem(stats["partition_method"])
        )
        ...
        self.comparison_table.setItem(
            row, 4, QTableWidgetItem(f"{stats['avg_distance']:.2f}")
        )
        self.comparison_table.setItem(
            row, 5, QTableWidgetItem(f"{stats['max_distance']:.2f}")
        )
        self.comparison_table.setItem(
            row, 6, QTableWidgetItem(f"{stats['min_distance']:.2f}")
        )
        self.comparison_table.setItem(
            row, 7, QTableWidgetItem(f"{stats['total_time']:.3f}")
        )
    ...
    def finish_comparison(self):
        ...
        fastest_result = min(
            self.comparison_results, key=lambda x: x["stats"]["total_time"]
        )
